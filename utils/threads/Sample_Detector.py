...
from utils.sample_structure.sample import Sample
from utils.suppport_scripts.marker_detection import find_marker_origin_conventional, square_image
from utils.suppport_scripts.qd_detection import get_qd_positions_from_image
...
from utils.suppport_scripts.qd_detector import get_image_qd
...
class SampleDetector(QObject):
    ...
    def detect_dots_from_manual_images(self, marker_type_dark=True):
        ...
        if self.sample is None:
            self.sample = Sample(name=sample_name,
                                 n_cells_x=np.sqrt(self.qd_analysis_parameter["n_cells"]),
                                 n_cells_y=np.sqrt(self.qd_analysis_parameter["n_cells"]),
                                 n_mfs_x=np.sqrt(self.qd_analysis_parameter["n_marker_fields"]),
                                 n_mfs_y=np.sqrt(self.qd_analysis_parameter["n_marker_fields"]),
                                 mf_size_x=mf_size,mf_size_y=mf_size,
                                 inter_cell_distance_x=self.qd_analysis_parameter["inter_cell_distance"],
                                 inter_cell_distance_y=self.qd_analysis_parameter["inter_cell_distance"])
    ...
    def calc_measurement_precision_error(self, data, sample_name="noname", fit_lim=250, read_fits=False, marker_type_dark=True,
                                         save_as_text=True, plot_results=False, verbose=False):
