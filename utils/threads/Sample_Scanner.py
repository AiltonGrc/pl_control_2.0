...
class SampleScanner(QObject):
    ...
    def scanning_sample(self, save_positions_in_file=False):
        ...
        for cell_y in range(int(np.sqrt(self.sample.n_cells))):
            # follow cells in S-form to minimize required travel distance
            if cell_y % 2 == 0:
                cell_order_x = range(0, int(np.sqrt(self.sample.n_cells)), 1)
                mf_order_x = range(0, int(np.sqrt(self.sample.n_marker_fields)), 1)
                direction_x = 1
            else:
                cell_order_x = range(int(np.sqrt(self.sample.n_cells)) - 1, -1, -1)
                mf_order_x = range(int(np.sqrt(self.sample.n_marker_fields)) - 1, -1, -1)
                direction_x = -1
            ...
            for n_cx, cell_x in enumerate(cell_order_x):
                ...
                for n_mfx, mf_x in enumerate(mf_order_x):
                    if mf_x % 2 == 0:
                        ...
                        mf_order_y = range(0, int(np.sqrt(self.sample.n_marker_fields)), 1)
                        direction_factor_mf_y = 1 * direction_x
                    else:
                        mf_order_y = range(int(np.sqrt(self.sample.n_marker_fields)) - 1, -1, -1)
                        direction_factor_mf_y = -1 * direction_x
    ...
    def qd_spectroscopy(self, n_splits_fss, cell_name, marker_field_name, use_only_search_pos=False, qd_detection_thresh=1.26, standalone_call=True):
        ...
        for n_qd, qd_pos in enumerate(abs_qd_pos_array):
            ...
            filename_noextension = f"{self.sample.name}_{cell_name}_{marker_field_name}_{n_qd}#Pol(deg)_0_180"
            filename = filename_noextension + '.spe'

            # Copied from measurement-py from Kai
